...
def compute_GMM(config, full_flair_list, log_dir):
    ...
    #plt.title(f"PCA comp {n_components}, explained variance: {sum(pca.explained_variance_ratio_):.4}")
    plt.axis('tight')
    plt.savefig(os.path.join(log_dir, config["part_file"]+"Institution_train_val.svg"))
    #plt.show()
...
def main(args, config):
    ...
    for cluster in range(config["gmm_components"]):
        ...
        train_length_dict[cluster] = 0
        for client in clustered_train_part_list[cluster]:
            train_ds_dict[cluster][client] = CacheDataset(data=clustered_train_part_list[cluster][client], transform=train_transform)
            train_loader_dict[cluster][client] = DataLoader(train_ds_dict[cluster][client], batch_size=config["batch_size"], shuffle=True, num_workers=0)
            train_length_dict[cluster] += len(train_ds_dict[cluster][client])
    
    for cluster in range(config["gmm_components"]):
        val_ds_dict[cluster] = {}
        val_loader_dict[cluster] = {}
        val_length_dict[cluster] = 0
        for client in clustered_val_part_list[cluster]:
            val_ds_dict[cluster][client] = CacheDataset(data=clustered_val_part_list[cluster][client], transform=val_transform)
            val_loader_dict[cluster][client] = DataLoader(val_ds_dict[cluster][client], batch_size=config["batch_size"], shuffle=False, num_workers=0)
            val_length_dict[cluster] += len(val_ds_dict[cluster][client])
    ...
    local_models = {}
    for cluster in range(config["gmm_components"]):
        local_models[cluster] = {}
        for client in clustered_train_part_list[cluster]:
            local_models[cluster][client] = DynUNet(
                                                spatial_dims = 3,
                                                in_channels = 4,
                                                out_channels = 3,
                                                kernel_size = (3, 3, 3, 3),
    ...
    optimizer_dict = {}
    for cluster in range(config["gmm_components"]):
        optimizer_dict[cluster] = {}
        for client in clustered_train_part_list[cluster]:
            if config["optim"] == "sgd":
                optimizer_dict[cluster][client] = torch.optim.SGD(local_models[cluster][client].parameters(), config["learning_rate"], weight_decay=config["weight_decay"])
            else:
                optimizer_dict[cluster][client] = torch.optim.Adam(local_models[cluster][client].parameters(), config["learning_rate"], weight_decay=config["weight_decay"])
    ...
    for cluster in range(config["gmm_components"]):
        ...
        # Local metrics
        for client in val_loader_dict[cluster].keys():
            
            with torch.no_grad(): 
                
                for (idx, val_data) in enumerate(tqdm(val_loader_dict[cluster][client])):
        ...
        for comm in range(config["max_comm_rounds"]):
            ...
            for client in local_models[cluster].keys():
                for local_param, cluster_param in zip(local_models[cluster][client].parameters(), cluster_models[cluster].parameters()):
            ...
            for client in local_models[cluster].keys():
                ...
                for local_epoch in range(config["max_local_epochs"]):
                    ...
                    local_models[cluster][client].train()
                    ...
                    for (idx, batch_data) in enumerate(tqdm(train_loader_dict[cluster][client])):
                        ...
                        optimizer_dict[cluster][client].zero_grad()
                        outputs = local_models[cluster][client](inputs)
                        
                        # Plot predictions
                        if comm==0 and local_epoch==0 and idx<4 and False:
                            with torch.no_grad():
                                fig = plot_train_image_pred_output(inputs, outputs, labels)
                                writer.add_figure(f"Initial check/Cluster {cluster}, Client {client}", fig, comm+1, close=False)
                                plt.show()
                        ...
                        loss.backward()
                        optimizer_dict[cluster][client].step()
                    ...
                    # lr_scheduler.step()
                    local_epoch_loss /= len(train_ds_dict[cluster][client])
        
                    writer.add_scalar(f"Loss/train/Cluster {cluster}, Client {client}", local_epoch_loss, epoch+1)
                    print(f"Cluster {cluster}, Client {client}, local epoch {local_epoch + 1}, average loss: {local_epoch_loss:.4f}")
            ...
            for client in local_models[cluster].keys():  
                ...
                for cluster_param, client_param in zip(cluster_models[cluster].parameters(), local_models[cluster][client].parameters()):
                    cluster_param.data += client_param.data.clone() * len(train_ds_dict[cluster][client])/train_length_dict[cluster]
            ...
            if (comm+1) % config["global_validation_interval"] == 0:
                ...
                writer.add_scalar(f"Validation/Cluster {cluster} model/Mean_dice", metric_d, comm+1)
                writer.add_scalar(f"Validation/Cluster {cluster} model/Tumor core dice", metric_tc, comm+1)
                writer.add_scalar(f"Validation/Cluster {cluster} model/Whole tumor dice", metric_wt, comm+1)
                writer.add_scalar(f"Validation/Cluster {cluster} model/Enhancing tumor dice", metric_et, comm+1)
